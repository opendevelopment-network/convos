...
const updateUser = user.api.operation('updateUser');
...
async function onSubmit(e) {
  ...
  if (!form.notifications.checked) {
    $user.enableNotifications = 'default';
  }
  else if ($user.enableNotifications != 'granted') {
    $user.enableNotifications = 'pending';
    Notification.requestPermission(status => {
      $user.enableNotifications = status;
    });
  }

  if (passwords.join('').length && passwords[0] != passwords[1]) {
    return updateUser.error('Passwords does not match.');
  }

  $user.expandUrlToMedia = form.expand_url.checked;
  updateUser.execute(e.target);
}
...
$: if (formEl && $user.highlightKeywords) formEl.highlight_keywords.value = $user.highlightKeywords;
</script>
...
<main class="main-app-pane align-content-middle">
  <h1>{l('Settings')}</h1>
  <form method="post" on:submit|preventDefault="{onSubmit}" bind:this="{formEl}">
